...
public class UI {
	...
	public boolean typingName = true;
	...
	public void draw(Graphics2D g2) {
		...
		if(gp.gameState == gp.lanState) {
			drawLanUI();
		}
		if(gp.gameState == gp.connectingState) {
		    drawConnecting();
		}
	...
	public void drawLvl() {
		...
		//TODO ADD REVEAL LATER
	    boolean isReveal = (gp.currentMap == 10 || gp.currentMap == 11 || gp.currentMap == 12); // only map 1 is reveal
	...
	public void drawLanUI() {
	    ...
	    g2.setFont(aldrich.deriveFont(60F));

	    String text = "LAN MODE";
	    int x = getXforCenteredText(text);
	    int y = gp.tileSize * 3;
	    g2.drawString(text, x, y);
	    ...
	    y += gp.tileSize * 3;
	    g2.drawString(text, x, y);
	    if(lanMenuNum == 0) g2.drawString(">", x - gp.tileSize, y);

	    text = "JOIN GAME";
	    x = getXforCenteredText(text);
	    y += gp.tileSize;
	    g2.drawString(text, x, y);
	    if(lanMenuNum == 1) g2.drawString(">", x - gp.tileSize, y);

	    text = "BACK";
	    x = getXforCenteredText(text);
	    y += gp.tileSize;
	    g2.drawString(text, x, y);
	    if(lanMenuNum == 2) g2.drawString(">", x - gp.tileSize, y);
	}
	...
	public void drawEndGameScreen() {
	    ...
	    g2.setFont(aldrich.deriveFont(80F));
	    String text = "YOU FINISHED THE GAME";
	    int x = getXforCenteredText(text);
	    int y = gp.tileSize * 4;
	    g2.drawString(text, x, y);
