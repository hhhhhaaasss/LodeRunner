package entity;
//Super Class for all entities of the game
//Abstract Class (Blueprint)

import Main.GamePanel;
import ai.Node;

import java.awt.Graphics2D;
import java.awt.Rectangle;
import java.awt.image.BufferedImage;

public class Entity {

	GamePanel gp;
	public int x,y;
	public int speed;
	
	//BufferedImage describes an Image with an accessible buffer or image data
	//We use this to store our image files
	public BufferedImage up1,up2,down1,down2,left1,left2,right1,right2, ropeLeft1, ropeLeft2, ropeRight1, ropeRight2, reveal;
	public String direction;
	
	//Walking animation
	public int spriteCounter = 0;
	public int spriteNum = 1;
	
	//Collision
	public Rectangle solidArea = new Rectangle(0,0,48,48); // takes X, Y , Width, Height
	public int solidAreaDefaultX, solidAreaDefaultY;
	public int collisionOn = 0;
	
	//Action for the NPC
	public int actionLockCounter = 0;
	
	public boolean onPath = true;
	
	boolean contactPlayer = false;
	boolean wasTouchingPlayer = false;
	
	public Entity(GamePanel gp) {
		this.gp = gp;
	}
	
	public void setAction() {}
	
	public void checkCollision() {
	    collisionOn = 0;
	    gp.gravity.CheckGravity(this);
	    boolean isTouchingPlayer = false;
	
	    isTouchingPlayer = gp.cChecker.checkPlayer(this);
	   

	    if (isTouchingPlayer && !wasTouchingPlayer) {
	    	
	    	
	    	gp.player.life--;

	        gp.gameState = gp.gameOverState;
	        gp.stopMusic();
	        gp.playMusic(6);
	        if (gp.player.life < 0) {
	            gp.player.life = 0;
	        }      
	    }

	    contactPlayer = isTouchingPlayer;
	    wasTouchingPlayer = isTouchingPlayer;
	}
	public void update() {
	    setAction();
	    int oldY = y;
	    checkCollision();
	    if (y != oldY) {
	        return; // gravity acted, skip movement
	    }
	    if (collisionOn != 1) {
	        switch(direction) {
	        case "up": y-= speed; break;
	        case "down": y += speed; break;
	        case "left": x -= speed; break;
	        case "right": x+= speed; break;
	        }
	    }
	    
	  //Update Walking animation
	  		spriteCounter++;
	  		if(spriteCounter > 12) { //Speed for the change
	  			if(spriteNum == 1) {
	  				spriteNum = 2;
	  			}else if(spriteNum == 2) {
	  				spriteNum = 1;
	  			}
	  			spriteCounter = 0;
	  		}
	}

	public void searchPath(int goalCol, int goalRow) {
	    int startCol = (x + solidArea.x + gp.tileSize / 2) / gp.tileSize;
	    int startRow = (y + solidArea.y + gp.tileSize / 2) / gp.tileSize;
	    gp.pFinder.setNodes(startCol, startRow, goalCol, goalRow);
	    if (gp.pFinder.search() && !gp.pFinder.pathList.isEmpty()) {
	        Node nextNode = gp.pFinder.pathList.get(0);
	        int entityCenterX = x + solidArea.x + solidArea.width / 2;
	        int entityCenterY = y + solidArea.y + solidArea.height / 2;
	        int nextCenterX = nextNode.col * gp.tileSize + gp.tileSize / 2;
	        int nextCenterY = nextNode.row * gp.tileSize + gp.tileSize / 2;
	        int tolerance = Math.max(1, speed);
	        if (nextNode.row == startRow && Math.abs(entityCenterY - nextCenterY) > tolerance) {
	            return;
	        }
	        if (nextNode.col == startCol && Math.abs(entityCenterX - nextCenterX) > tolerance) {
	            return;
	        }
	        if (nextCenterX < entityCenterX) direction = "left";
	        else if (nextCenterX > entityCenterX) direction = "right";
	        else if (nextCenterY < entityCenterY) direction = "up";
	        else if (nextCenterY > entityCenterY) direction = "down";
	    }
	}
	
	public void draw(Graphics2D g2) {
		
		BufferedImage image = null;
		switch(direction) {
		
		case "up":
			
			if(spriteNum == 1) image = up1;
			if(spriteNum == 2) image = up2;
			
			break;
		case "down":
			
			if(spriteNum == 1) image = down1;
			if(spriteNum == 2) image = down2;
			
			break;
		case "left":
			
			if(spriteNum == 1) image = left1;
			if(spriteNum == 2) image = left2;
			
			break;
		case "right":
			
			if(spriteNum == 1) image = right1;
			if(spriteNum == 2) image = right2;
			
			break;
		}
		
		g2.drawImage(image, x, y, gp.tileSize, gp.tileSize, null);
	}
	
}
